...
| `EVAL-UI-STATES-001` | #80 | Implemented | Twenty-seven synthetic fixtures and the 15-screen matrix enforce six shared states, explicit subject selection, retained career context, consistent identity, partial/conflicting fields, safe freshness, redacted settings, closed text and sanitized failures without production dependencies | `scripts/validate_ui_fixtures.py`, `tests/test_ui_state_fixtures.py`, `woff/tests/test_architecture_contracts.py` |
